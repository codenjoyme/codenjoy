...
public class GameClientResolver {
  ...
  @PostConstruct
  void init() {
    gameProperties.getServers().forEach(server -> {
      clientsByLocation.put(server.getLocation(), buildGameServerClient(server));
    });
  }
  ...
  private GameServerClient buildGameServerClient(ServerEntry server) {
    return Feign.builder()
        ...
        .requestInterceptor(new BasicAuthRequestInterceptor(server.getBasicAuthUser(), server.getBasicAuthPassword()))
        .target(GameServerClient.class, server.getSchema() + "://" + server.getLocation());
  }
