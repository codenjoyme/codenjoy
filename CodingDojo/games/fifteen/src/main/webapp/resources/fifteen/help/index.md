...
Итак, предстоит написать своего бота для пятнашек.   
Герой может передвигаться во все четыре стороны, Меняясь с соседней цифрой.

Цель - сложить пятнашки. Если все цифры на своих местах, игра выиграна - 
начисляются очки. За каждую цифру, которую поставили на свое место герой получит
бонусные[*](index.md#ask) очки. 
...
В этом примере — позиция героя (символ `+`) — `[4, 4]`.
...
* `UP`, `DOWN`, `LEFT`, `RIGHT` – приводят к движению героя
  в заданном направлении на 1 клетку (герой меняется местами с цифрой).
...
* Обменяться герою местами со случайной соседней цифрой.
* Переместить цифру 1 на свое место.
* Переместить цифру 2 на свое место, не меняя положения цифры 1.
* Переместить цифру 3 на свое место, не меняя положения цифр 1 и 2.
