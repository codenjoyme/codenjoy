...
Далее необходимо подключиться [из кода](../../../resources/sokoban/user/clients.zip)
к серверу через websocket. Это Maven проект и подойдет он дял игры на JVM языках.
Как его запустить смотри в корне README.txt
