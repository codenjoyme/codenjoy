...
Нужно написать своего бота который перемести все боксы в пустые тайлы 
что бы их заполнить. Можно двигаться во все 4 стороны, а так же менять 
коробку которой ты управляешь в данный момент времени.

После того как ты дал команду движения коробке, выбраная коробка
будет двигаться в том направлении пока не попадет в поле которое
нужно заполнить или же не упрется в стену. После того как коробка 
упирается в стену её можно двигать снова. Но помни, чем больше ходов 
ты делаешь для того что бы доставить коробку в пункт назначения, 
тем меньше очков ты получишь.

Побеждает игрок доставивший все коробочки куда нужно за минимальное количество ходов.
...
левом верхнем углу и имеет координату `[0, 10]`.
Координата `[0, 0]` соответствует левому нижнему углу. 
В этом примере — позиция коробки (символ `#`) — `[1, 1]`.
...
* `UP`, `DOWN`, `LEFT`, `RIGHT` – приводят к движению героя
  в заданном направлении до попадания в Target или пока не попадет в стену. 
* `ACT` - Сменить текущую коробочку.
...
* Переместить любую коробку в любое место.
* Выбрать другую коробку из набора существующих.
* Рассчитать какая коробка куда должна направиться.
