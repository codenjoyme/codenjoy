...
public class Chance<T extends CharElements> {
    ...
    public static final SettingsReader.Key RESERVED = () -> "[Chance] Reserved for autogenerated";
    ...
    public void checkReservedForAuto() {
        try {
            settings.integer(RESERVED);
        } catch (Exception e) {
            // если его не установили, сделаем это сами
            settings.integer(RESERVED, DEFAULT_RESERVED_VALUE);
        }
    ...
    private int reservedForAuto() {
        return settings.integer(RESERVED);
    }
