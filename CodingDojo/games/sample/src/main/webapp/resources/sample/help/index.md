...
Далее необходимо подключиться [из кода](../../../resources/sample/user/clients.zip) 
...
Формат таков `^board=(.*)$`. C помощью этого regexp можно 
выкусить строку доски.
...
строки каждые `sqrt(length(string))` символов, 
...
(боту) состояние обновленного поля на текущий момент 
и ожидает ответа команды герою. За следующую секунду игрок 
должен успеть дать команду герою. Если не успел — герой 
стоит на месте.
...
* `ACT` - оставить бомбу. Команды движения можно комбинировать  
   ...
   что за один такт игры будет оставлена бомба, 
   а потом движение `LEFT, ACT` или наоборот `ACT, LEFT`.

Первостепенная задача – написать websocket клиента, который 
