...
по очкам. Каждый играет на своем поле. Игрок может вращать любую ячейку
командой `ACT(x,y` - повернув тем самым по часовой стрелке трубу находящуюся там.

Если при этом на карте образуется зацикленная труба - она удалится из поля,
а игрок получит призовые очки[*](index.md#ask). За каждый ход в свободных 
местах поля случайным образом появляются трубы различной формы. 
Если все поле заполнено трубами - поле очищается и игрок получает 
штрафные очки[*](index.md#ask). 
...
* Повернуть случайную трубу в любую сторону.
* Поворачивать трубы так, чтобы когда появится достаточное их число - иметь возможность 
  быстро их замкнуть, освободив тем самым место на поле. 
