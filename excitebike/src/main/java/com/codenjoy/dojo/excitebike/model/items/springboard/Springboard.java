...
public class Springboard extends PointImpl implements Shiftable {
    private int width;
    private int x0;
    private List<SpringboardElement> riseElements;
    private List<SpringboardElement> betweenElements;
    private List<SpringboardElement> descentElements;

    public Springboard(int x, int linesNumber, int width) {
        super(x + width + 2, linesNumber);
        x0 = x;
        this.width = width;
    ...
    private List<SpringboardElement> getDescentLine(int linesNumber) {
        return generateSpringBoardLine(SPRINGBOARD_LEFT_UP, SPRINGBOARD_DARK, SPRINGBOARD_LEFT_DOWN, getX(), linesNumber);
    }
    ...
    public List<SpringboardElement> getElements() {
        return new ArrayList<SpringboardElement>() {
            {
                addAll(riseElements);
